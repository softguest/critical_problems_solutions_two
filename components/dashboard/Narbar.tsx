...
export default function Navbar() {
  ...
  const session = { user: { firstName: "User" } };

  return (
    <header className="sticky top-0 z-10 w-full bg-white shadow-sm px-4 py-2 flex items-center justify-between md:pl-4">
      ...
      <UserModal showModal={showModal} setShowModal={setShowModal} firstName={session?.user?.firstName || 'User'} />
    </header>
  );
}
